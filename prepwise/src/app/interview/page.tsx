...
export default function InterviewPage() {
  ...
  const handleInterviewComplete = async (responses: RecordedResponse[]) => {
    ...
    try {
      ...
      // Save recording to history (if user is authenticated)
      try {
        const saveResponse = await authenticatedFetch("/api/interviews/save", {
          method: "POST",
          body: JSON.stringify({
            sessionId: sessionPlan.sessionId,
            candidateId,
        ...
        if (!saveResponse.ok) {
          ...
          console.warn("Failed to save interview to history:", errorData.error || saveResponse.statusText);
        } else {
          console.log("Interview saved to Supabase successfully");
        }
      } catch (saveError) {
        console.warn("Failed to save interview to history:", saveError);
              ...
              {!sessionPlan && (
                <section className="rounded-3xl border border-slate-200 bg-white p-6 shadow-lg">
                  ...
                  <p className="mt-1 text-sm text-slate-600">
                    {userTier === "premium" || userTier === "enterprise"
                      ? "AI-powered personalized questions tailored to your unique background and target schools."
                      : "We tailor questions across leadership, execution, and school fit dimensions."}
                  </p>
                  {userTier === "free" && (
                    <div className="mt-3 rounded-lg border border-amber-200 bg-amber-50 p-3">
                      <p className="text-xs text-amber-800">
                        💡 <strong>Upgrade to Premium</strong> for highly personalized questions with advanced AI analysis.
                      </p>
                      <a
                        href="/pricing"
